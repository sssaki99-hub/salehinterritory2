...
if (!supabaseUrl || !supabaseAnonKey || supabaseUrl === 'YOUR_SUPABASE_URL') {
    alert("Supabase URL and Anon Key must be provided in supabaseClient.ts. The application will not work correctly until you update these values.");
    console.error("Supabase URL and Anon Key must be provided in supabaseClient.ts.");
}
...
export const getSettings = async (): Promise<AdminSettings> => (await supabase.from('settings').select('*').eq('id', 1).single()).data;
...
export const addComment = (c: Omit<Comment, 'id'|'timestamp'>, postId: string, type: 'project'|'writing') => addItem('comments', { author: c.author, text: c.text, project_id: type === 'project' ? postId : null, writing_id: type === 'writing' ? postId : null });
export const addRating = (r: Rating, postId: string, type: 'project'|'writing') => addItem('ratings', { value: r.value, project_id: type === 'project' ? postId : null, writing_id: type === 'writing' ? postId : null });
