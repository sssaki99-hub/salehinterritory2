...
const Header: React.FC = () => {
  ...
  const navLinks = [
    ...
    { name: 'Professional Details', path: '/professional' },
    { name: 'Contact', path: '/contact' },
    { name: 'Admin', path: '/admin' },
  ];

  const linkClasses = "px-3 py-2 rounded-md text-sm font-medium transition-colors duration-300";
  const activeLinkClasses = "bg-gray-800 text-primary-accent";
  const inactiveLinkClasses = "text-gray-300 hover:bg-gray-700 hover:text-white";

  return (
    <header className="bg-dark-bg/70 backdrop-blur-md sticky top-0 z-50 shadow-lg shadow-primary-accent/10">
      <nav className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center">
            <NavLink to="/" className="text-white font-serif text-2xl font-bold text-indigo-400">
              Salehin's Territory
            </NavLink>
          </div>
